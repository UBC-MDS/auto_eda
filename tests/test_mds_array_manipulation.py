...
def test_single_element_array_1(one_d_int_array_one_element):
    assert np.array_equal(am.sort_array(one_d_int_array_one_element), one_d_int_array_one_element)

def test_single_element_array_2(one_d_string_array_one_element):
    assert np.array_equal(am.sort_array(one_d_string_array_one_element), one_d_string_array_one_element)

# checks if an empty array is returned as is
def test_empty_array(empty_array):
    assert np.array_equal(am.sort_array(empty_array), empty_array)
...
def test_negative_input(one_d_neg_array):
    assert np.array_equal(am.sort_array(one_d_neg_array), np.array(sorted(one_d_neg_array)))
...
def test_lowercase_uppercase_strings(one_d_string_array_mixed_case):
    assert np.array_equal(am.sort_array(one_d_string_array_mixed_case), np.array(sorted(one_d_string_array_mixed_case, key=str.lower)))

# checks if a sorted array remains unchanged after sorting again
def test_already_sorted_array(one_d_int_array_sorted):
    assert np.array_equal(am.sort_array(one_d_int_array_sorted), one_d_int_array_sorted)